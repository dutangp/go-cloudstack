...
func (p *DeployVirtualMachineParams) toURLValues() url.Values {
	...
	if v, found := p.p["productcode"]; found {
		u.Set("productcode", v.(string))
	}
...
func (p *DeployVirtualMachineParams) SetProductcode(v string) {
	if p.p == nil {
		p.p = make(map[string]interface{})
	}
	p.p["productcode"] = v
...
func (p *DeployVirtualMachineParams) SetPasswordenabled(v string) {
  if p.p == nil {
    p.p = make(map[string]interface{})
  }
...
func (s *VirtualMachineService) DestroyVirtualMachine(p *DestroyVirtualMachineParams) (*DestroyVirtualMachineResponse, error) {
	...
	resp, err := s.cs.newRequest("tmDestroyVirtualMachine", p.toURLValues())
	if err != nil {
		return nil, err
	}
